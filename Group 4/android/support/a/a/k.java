/*
 * Decompiled with CFR 0_115.
 */
package android.support.a.a;

import android.annotation.TargetApi;
import android.content.res.Resources;
import android.content.res.TypedArray;
import android.graphics.ColorFilter;
import android.graphics.PorterDuff;
import android.graphics.Rect;
import android.graphics.Region;
import android.graphics.drawable.Drawable;
import android.support.v4.c.a.a;
import android.util.AttributeSet;

@TargetApi(value=21)
abstract class k
extends Drawable {
    Drawable a;

    k() {
    }

    static TypedArray b(Resources resources, Resources.Theme theme, AttributeSet attributeSet, int[] arrn) {
        if (theme != null) return theme.obtainStyledAttributes(attributeSet, arrn, 0, 0);
        return resources.obtainAttributes(attributeSet, arrn);
    }

    public void applyTheme(Resources.Theme theme) {
        if (this.a == null) return;
        a.a(this.a, theme);
    }

    public void clearColorFilter() {
        if (this.a != null) {
            this.a.clearColorFilter();
            return;
        }
        super.clearColorFilter();
    }

    public ColorFilter getColorFilter() {
        if (this.a == null) return null;
        return a.e(this.a);
    }

    public Drawable getCurrent() {
        if (this.a == null) return super.getCurrent();
        return this.a.getCurrent();
    }

    public int getLayoutDirection() {
        if (this.a == null) return 0;
        a.g(this.a);
        return 0;
    }

    public int getMinimumHeight() {
        if (this.a == null) return super.getMinimumHeight();
        return this.a.getMinimumHeight();
    }

    public int getMinimumWidth() {
        if (this.a == null) return super.getMinimumWidth();
        return this.a.getMinimumWidth();
    }

    public boolean getPadding(Rect rect) {
        if (this.a == null) return super.getPadding(rect);
        return this.a.getPadding(rect);
    }

    public int[] getState() {
        if (this.a == null) return super.getState();
        return this.a.getState();
    }

    public Region getTransparentRegion() {
        if (this.a == null) return super.getTransparentRegion();
        return this.a.getTransparentRegion();
    }

    public boolean isAutoMirrored() {
        if (this.a == null) return false;
        a.b(this.a);
        return false;
    }

    public void jumpToCurrentState() {
        if (this.a == null) return;
        a.a(this.a);
    }

    protected void onBoundsChange(Rect rect) {
        if (this.a != null) {
            this.a.setBounds(rect);
            return;
        }
        super.onBoundsChange(rect);
    }

    protected boolean onLevelChange(int n2) {
        if (this.a == null) return super.onLevelChange(n2);
        return this.a.setLevel(n2);
    }

    public void setAutoMirrored(boolean bl2) {
        if (this.a == null) return;
        a.a(this.a, bl2);
    }

    public void setChangingConfigurations(int n2) {
        if (this.a != null) {
            this.a.setChangingConfigurations(n2);
            return;
        }
        super.setChangingConfigurations(n2);
    }

    public void setColorFilter(int n2, PorterDuff.Mode mode) {
        if (this.a != null) {
            this.a.setColorFilter(n2, mode);
            return;
        }
        super.setColorFilter(n2, mode);
    }

    public void setFilterBitmap(boolean bl2) {
        if (this.a == null) return;
        this.a.setFilterBitmap(bl2);
    }

    public void setHotspot(float f2, float f3) {
        if (this.a == null) return;
        a.a(this.a, f2, f3);
    }

    public void setHotspotBounds(int n2, int n3, int n4, int n5) {
        if (this.a == null) return;
        a.a(this.a, n2, n3, n4, n5);
    }

    public boolean setState(int[] arrn) {
        if (this.a == null) return super.setState(arrn);
        return this.a.setState(arrn);
    }
}

