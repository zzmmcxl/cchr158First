...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            ...
            s = deltaMaxMin / (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs((2.0f * l) - TextTrackStyle.DEFAULT_FONT_SCALE));
        }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float l = hsl[2];
        float c = (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs((2.0f * l) - TextTrackStyle.DEFAULT_FONT_SCALE)) * s;
        float m = l - (0.5f * c);
        float x = c * (TextTrackStyle.DEFAULT_FONT_SCALE - Math.abs(((h / BitmapDescriptorFactory.HUE_YELLOW) % 2.0f) - TextTrackStyle.DEFAULT_FONT_SCALE));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
