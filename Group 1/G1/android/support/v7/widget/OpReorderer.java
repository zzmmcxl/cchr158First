...
class OpReorderer
{
    ...
    private void swapMoveOp(final List<AdapterHelper.UpdateOp> list, final int n, final int n2) {
        final AdapterHelper.UpdateOp updateOp = (AdapterHelper.UpdateOp)list.get(n);
        final AdapterHelper.UpdateOp updateOp2 = (AdapterHelper.UpdateOp)list.get(n2);
    ...
    void swapMoveRemove(final List<AdapterHelper.UpdateOp> list, final int n, final AdapterHelper.UpdateOp updateOp, final int n2, final AdapterHelper.UpdateOp updateOp2) {
        ...
        if (updateOp.itemCount < updateOp2.positionStart) {
            ...
            if (updateOp2.itemCount == 0) {
                list.remove(n2);
                mCallback.recycleUpdateOp(updateOp2);
            }
            return;
        }
        if (updateOp.positionStart <= updateOp2.positionStart) {
            ...
            if (positionStart4 < n7) {
                obtainUpdateOp = mCallback.obtainUpdateOp(2, 1 + updateOp.positionStart, updateOp2.positionStart + updateOp2.itemCount - updateOp.positionStart, null);
    ...
    void swapMoveUpdate(final List<AdapterHelper.UpdateOp> list, final int n, final AdapterHelper.UpdateOp updateOp, final int n2, final AdapterHelper.UpdateOp updateOp2) {
        ...
        if (updateOp.itemCount < updateOp2.positionStart) {
            ...
            if (itemCount < n3) {
                --updateOp2.itemCount;
                obtainUpdateOp = mCallback.obtainUpdateOp(4, updateOp.positionStart, 1, updateOp2.payload);
            }
        }
        if (updateOp.positionStart <= updateOp2.positionStart) {
            ++updateOp2.positionStart;
            ...
            if (positionStart < n4) {
                ...
                obtainUpdateOp2 = mCallback.obtainUpdateOp(4, 1 + updateOp.positionStart, n5, updateOp2.payload);
