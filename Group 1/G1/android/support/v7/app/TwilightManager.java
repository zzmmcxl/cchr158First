...
class TwilightManager
{
    ...
    private Location getLastKnownLocation() {
        final int checkSelfPermission = PermissionChecker.checkSelfPermission(mContext, "android.permission.ACCESS_COARSE_LOCATION");
        Location lastKnownLocationForProvider = null;
        if (checkSelfPermission == 0) {
            lastKnownLocationForProvider = this.getLastKnownLocationForProvider("network");
        }
        final int checkSelfPermission2 = PermissionChecker.checkSelfPermission(mContext, "android.permission.ACCESS_FINE_LOCATION");
        ...
        if (mLocationManager != null) {
            try {
                if (mLocationManager.isProviderEnabled(s)) {
                    return mLocationManager.getLastKnownLocation(s);
                }
            }
            catch (Exception ex) {
