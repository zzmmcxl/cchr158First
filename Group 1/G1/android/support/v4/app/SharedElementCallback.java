...
public abstract class SharedElementCallback
{
    ...
    private static Bitmap createDrawableBitmap(final Drawable drawable) {
        ...
        final float min = Math.min(1.0f, (float)MAX_IMAGE_SIZE / (float)(intrinsicWidth * intrinsicHeight));
    ...
    public Parcelable onCaptureSharedElementSnapshot(final View view, final Matrix matrix, final RectF rectF) {
        ...
        if (round > 0) {
            bitmap = null;
            if (round2 > 0) {
                final float min = Math.min(1.0f, (float)MAX_IMAGE_SIZE / (float)(round * round2));
